...
//				_rootGroup = (PBXDictionary)_objects[ (string)_rootObject[ "mainGroup" ] ];
        ...
        public bool AddCodeSigningEntitlements(string codeSign)
        {
            foreach (KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations)
            {
                buildConfig.Value.AddCodeSignEntitlements(codeSign);
            }
            modified = true;
            return modified;    
        }

        public bool AddEnableBitCode(bool bitCode)
        {
            foreach (KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations)
            {
                buildConfig.Value.AddEnableBitCode(bitCode);
            }
            modified = true;
            return modified;    
        }
        ...
        public bool GccEnableObjCExceptions(bool value)
        {
            foreach (KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations)
            {
		...
		{
			project.AddPushSystemCapabilities(value);
...
//				PBXDictionary<PBXBuildPhase> currentPhase = GetBuildPhase( fileReference.buildPhase );
        ...
        public bool AddFolder(string folderPath, PBXGroup parent = null, string[] exclude = null, bool recursive = true, bool createBuildFile = true, string flag = null)
        {
...
//			groups.Add( newGroup );
            ...
            foreach (string directory in Directory.GetDirectories( folderPath ))
            {
                ...
                if (directory.EndsWith("language"))
                {
                    sourceDirectoryInfo = new DirectoryInfo(directory);
                    System.Uri fileURI = new System.Uri(directory);
                    System.Uri rootURI = new System.Uri((projectRootPath + "/."));
                    string filePath = rootURI.MakeRelativeUri(fileURI).ToString();
                    AddLocalization(directory, GetGroup(sourceDirectoryInfo.Name, filePath, newGroup, "<group>"));
                    continue;
                } else
                {
...
//    					Debug.LogWarning( "This is a special folder: " + directory );
                        AddFile(directory, null, newGroup, "SOURCE_ROOT", createBuildFile, false, flag);
                        continue;
                    }
                    ...
                    {
//    					Debug.Log( "recursive" );
						AddFolder(directory, newGroup, exclude, recursive, createBuildFile, flag);
                    }
                }
            }
            // Adding files.
            foreach (string file in Directory.GetFiles( folderPath ))
            {
                ...
                AddFile(file, null, newGroup, "SOURCE_ROOT", createBuildFile, false, flag);
            }
        ...
        public PBXGroup GetGroup(string name, string path = null, PBXGroup parent = null, string tree = "SOURCE_ROOT")
        {
...
//				Debug.Log( "current: " + current.Value.guid + ", " + current.Value.name + ", " + current.Value.path + " - " + parent.HasChild( current.Key ) );
            ...
            PBXGroup result = new PBXGroup(name, path, tree);
            groups.Add(result);
        ...
        public void Adjust()
        {
            // UIKit.framework, Foundation.framework optional 설정
            string key = null;
            foreach (KeyValuePair<string, PBXFileReference> current in fileReferences)
            {
                if (current.Value != null)
                {
                    ...
                    {
                        if (current.Value.name.Equals("UIKit.framework"))
                        {
                            key = current.Value.guid;
                        }
                        else if (current.Value.name.Equals("Foundation.framework"))
                        {
                            key = current.Value.guid;
                        }
                        ...
                        if (key != null)
                        {
                            foreach (KeyValuePair<string, PBXBuildFile> curr in buildFiles)
                            {
                                string fileRef = (string)curr.Value.data ["fileRef"];
                                if (key.Equals(fileRef))
                                {
                                    curr.Value.SetWeakLink(true);
                                }
                            }
                            key = null;
                        }
