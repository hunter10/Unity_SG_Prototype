...
//				Debug.Log( "adding: " + currentPath );
        ...
        // Code Signing 추가
        public bool AddCodeSignEntitlements(string codeSign)
        {
            bool modified = false;
            ...
            {
                ((PBXDictionary)_data [BUILDSETTINGS_KEY]).Add(CODE_SIGN_ENTITLEMENTS_KEY, codeSign);
                modified = true;
            } else
            {
                string existed = (string)((PBXDictionary)_data [BUILDSETTINGS_KEY]) [CODE_SIGN_ENTITLEMENTS_KEY];
                if (!string.Equals(existed, codeSign))
                {
                    ((PBXDictionary)_data [BUILDSETTINGS_KEY]) [CODE_SIGN_ENTITLEMENTS_KEY] = codeSign;
                    modified = true;
                }
            }
        ...
        public bool AddEnableBitCode(bool bitCode)
        {
            ...
            {
                ((PBXDictionary)_data [BUILDSETTINGS_KEY]).Add(ENABLE_BITCODE_KEY, code);
                modified = true;
            } else
            {
                string existed = (string)((PBXDictionary)_data [BUILDSETTINGS_KEY]) [ENABLE_BITCODE_KEY];
                if (!string.Equals(existed, code))
                {
                    ((PBXDictionary)_data [BUILDSETTINGS_KEY]) [ENABLE_BITCODE_KEY] = code;
                    modified = true;
                }
            }
        ...
        public bool GccEnableObjCExceptions(bool value)
        {
			...
			if (!((PBXDictionary)_data [BUILDSETTINGS_KEY]).ContainsKey(GCC_ENABLE_OBJC_EXCEPTIONS_KEY))
			{
				((PBXDictionary)_data [BUILDSETTINGS_KEY]).Add(GCC_ENABLE_OBJC_EXCEPTIONS_KEY, code);
				modified = true;
			} else
			{
				string existed = (string)((PBXDictionary)_data [BUILDSETTINGS_KEY]) [GCC_ENABLE_OBJC_EXCEPTIONS_KEY];
				if (!string.Equals(existed, code))
				{
					((PBXDictionary)_data [BUILDSETTINGS_KEY]) [GCC_ENABLE_OBJC_EXCEPTIONS_KEY] = code;
					modified = true;
				}
			}
			return modified;
